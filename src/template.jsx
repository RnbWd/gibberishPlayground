 ...
 getInitialState: function() {
  ...
  handleInputR: function(event) {
    var input = event.target.value;
     this.setState({right: input});
  },
   handleInputL: function(event) {
    var input = event.target.value;
      this.setState({left: input});
  },
  ...
  render: function() {
    ...
    (this.state.left != '') ? sineL.frequency = parseInt(value) + parseInt(this.state.left) : '';
    (this.state.right != '') ? sineR.frequency = parseInt(value) + parseInt(this.state.right) : '';

    var inputL = <input id="textInputL" type="text" className="form-control" value={this.state.left} onChange={this.handleInputL} />
    var inputR = <input id="textInputR" type="text" className="form-control" value={this.state.right} onChange={this.handleInputR} />

    var paragraph = <p id="awesome" className="text-center lead">Frequency: {value} hz</p>
    ...
    var left = <button className={(this.props.status) ? "btn btn-success pull-left play" : "btn btn-danger pull-left play"} onClick={this.playL} disabled={(this.props.status) ? false : true} >Left</button>
    var right = <button className={(this.props.status) ? "btn btn-success pull-right play" : "btn btn-danger pull-right play"} onClick={this.playR} disabled={(this.props.status) ? false : true} >Right</button>

    return (
      <div className="container-fluid" onWheel={this.handleScroll} >
...
var Filters = React.createClass({
  ...
  connect: function() {
   ...
   render: function() {
    ...
    var connect = <button style={style} className={(status == "connect") ? "btn btn-info connect" : "btn btn-warning connect"} onClick={this.connect} >{status}</button>
    return (
      <div>
        ...
        <h4>Sound</h4>
        <select className="form-control" value={this.state.optionS} onChange={this.handleSound}>
            <option value="a">Synth</option>
            <option value="b">Sine</option>
          </select>
          <h4 style={style}>Filters</h4>
          <select style={style} className="form-control" value={this.state.optionF} onChange={this.handleFilter}>
            <option value="a">None</option>
            <option value="b">Distortion</option>
            <option value="c">Delay</option>
            <option value="d">Decismator</option>
            <option value="e">Modulator</option>
            <option value="f">Reverb</option>
          </select>
        </div>
      </div>
    );
