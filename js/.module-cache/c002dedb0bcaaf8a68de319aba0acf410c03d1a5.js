...
var Center = React.createClass({displayName: 'Center',
 getInitialState: function() {
  ...
  play: function() {
    ...
    else if (this.props.sound === sine && this.state.sine == "off") {
      this.props.sound.connect();
      this.setState({sine: "on"});
    }
    else if (this.sine.state == "on") {
...
var Filters = React.createClass({displayName: 'Filters',
  getInitialState: function() {
    return {filter: '', sound: synth, optionF: "a", optionS: "b", status: "connect"}
  },
  ...
  connect: function() {
   ...
   render: function() {
    ...
    return (
      React.DOM.div(null, 
        Center( {status:(status == "connect") ? false : true, sound:  (this.state.filter != '') ? this.state.sound : this.state.filter} ),
        connect,
