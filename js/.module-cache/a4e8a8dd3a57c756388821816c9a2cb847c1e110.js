...
var Filters = React.createClass({displayName: 'Filters',
  getInitialState: function() {
    return {filter: '', sound: sine, optionF: "a", optionS: "b", status: "connect"}
  },
  ...
  connect: function() {
    ...
    if (this.state.status == "connect") {
      (this.state.sound === synth) ? this.state.sound.connect() : '';
      this.setState({status: "disconnect"});
      //(this.state.sound === sine) ? this.state.sound.disconnect() : '';
    } else {
      (this.state.sound === synth) ? this.state.sound.disconnect() : '';
...
/*
<select className="form-control" value={this.state.optionF} onChange={this.handleFilter}>
