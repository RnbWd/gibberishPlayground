...
var Filters = React.createClass({displayName: 'Filters',
  ...
  connect: function() {
    if (this.state.status == "connect") {
      ...
      (this.state.filter != '') ? this.state.filter.disconnect() : this.state.sound.connect();
    ...
    return (
      React.DOM.div(null, 
        ...
        React.DOM.div( {className:"filters"}, 
          ...
          React.DOM.select( {className:"form-control", value:this.state.sound, onChange:this.handleSound}, 
