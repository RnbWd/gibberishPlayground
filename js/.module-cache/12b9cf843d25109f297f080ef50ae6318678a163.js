...
var Center = React.createClass({displayName: 'Center',
 getInitialState: function() {
  ...
  render: function() {
    ...
    var play = React.DOM.button( {className:(value > 100 && value < 1600 && this.props.status) ? "btn btn-success play" : "btn btn-danger play", onClick:this.play, disabled:(value > 100 && value < 1600 && this.props.status) ? false : true} , "Play")
    return (
...
var Filters = React.createClass({displayName: 'Filters',
  ...
  connect: function() {
   ...
   render: function() {
    ...
    var connect = React.DOM.button( {className:(status == "connect") ? "btn btn-info btn-sm connect" : "btn btn-warning btn-sm connect", onClick:this.connect} , status)
    return (
      React.DOM.div(null, 
